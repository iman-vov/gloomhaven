...
function confirmLocalReset() {
  if (typeof sessionCode !== 'undefined' && sessionCode) return; // тільки локально
  const de = lang === 'de';
  const overlay = document.createElement('div');
  overlay.id = 'local-reset-overlay';
  overlay.innerHTML = `
    <div class="deck-view-box" style="max-width:340px;width:95%;">
      <h3 style="color:var(--red);">⚠ ${de ? 'Achtung!' : 'Внимание!'}</h3>
      <p style="margin:12px 0 18px;font-size:0.9rem;line-height:1.5;color:var(--text);">
        ${de
          ? 'Alle Fortschritte werden gelöscht:<br><strong>XP, Gold, Karten, Gegenstände → 0</strong>.<br>Diese Aktion kann nicht rückgängig gemacht werden.'
          : 'Будут сброшены все данные:<br><strong>XP, Золото, Карты, Предметы → 0</strong>.<br>Это действие нельзя отменить.'}
      </p>
      <div style="display:flex;gap:8px;">
        <button onclick="applyLocalReset()" style="flex:1;padding:10px;background:var(--red);border:none;border-radius:6px;color:#fff;font-family:'Cinzel',serif;font-size:0.8rem;cursor:pointer;">
          ${de ? 'Ja, zurücksetzen' : 'Да, сбросить'}
        </button>
        <button onclick="document.getElementById('local-reset-overlay').remove()" style="flex:1;padding:10px;background:var(--bg3);border:1px solid var(--border2);border-radius:6px;color:var(--text-dim);font-family:'Cinzel',serif;font-size:0.8rem;cursor:pointer;">
          ${de ? 'Abbrechen' : 'Отмена'}
        </button>
      </div>
    </div>`;
...
async function applyLocalReset() {
  const overlay = document.getElementById('local-reset-overlay');
  if (overlay) overlay.remove();

  const chars = ['HA', 'DE', 'VW', 'RG'];
  chars.forEach(code => {
    localStorage.removeItem('gh_player_' + code);
    localStorage.removeItem('gh_deck_' + code);
    localStorage.removeItem('gh_inventory_' + code);
  });
  localStorage.removeItem('activeChar');
  localStorage.removeItem('gh_gold');
  localStorage.removeItem('gh_mission');
